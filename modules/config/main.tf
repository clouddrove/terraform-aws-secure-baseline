...
resource "aws_iam_role" "recorder" {
  ...
  assume_role_policy = join("", data.aws_iam_policy_document.recorder_assume_role_policy.*.json)
...
resource "aws_iam_role_policy" "recorder_publish_policy" {
  ...
  policy = join("", data.aws_iam_policy_document.recorder_publish_policy.*.json)
}
...
resource "aws_config_configuration_recorder" "recorder" {
  ...
  name     = format("%s-recorder", module.labels.id)
  role_arn = var.enabled && var.only_config_enabled == false ? join("", aws_iam_role.recorder.*.arn) : var.config_role_arn
