...
// Description : This Terratest is used to test the Terraform alarm baseline  module.
...
func Test(t *testing.T) {
	...
	Arn := terraform.OutputList(t, terraformOptions, "cloudtrail_arn")

	// Check that we get back the outputs that we expect
	assert.Contains(t, Arn[0], "arn:aws:cloudtrail")
}
